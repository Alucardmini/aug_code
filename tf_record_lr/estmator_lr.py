...
def model_fn(features, labels, mode, params):
    ...
    X = tf.cast(features["img_raw"], tf.float32, name="input_image")
    ...
    # ==========  如果是 train 任务  ========== #
    # 构建train_op
    train_op = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=0.9, beta2=0.999, epsilon=1e-8).minimize(
...
def main(_):
    ...
    tr_files = "/Users/wuxikun/Documents/LetsAug/tf_record_lr/train.tfrecords"
    va_files = "/Users/wuxikun/Documents/LetsAug/tf_record_lr/train.tfrecords"
    te_files = "/Users/wuxikun/Documents/LetsAug/tf_record_lr/train.tfrecords"
    ...
    if task_type == "train":
        ...
        early_stop_hook = tf.estimator.experimental.stop_if_no_increase_hook(estimator, metric_name="accuracy",
                                                                             max_steps_without_increase=1000,
                                                                             min_steps=500)
        train_spec = tf.estimator.TrainSpec(input_fn=lambda: input_fn(tr_files, num_epochs=10, batch_size=32),
                                            hooks=[early_stop_hook])
        eval_spec = tf.estimator.EvalSpec(input_fn=lambda: input_fn(va_files, num_epochs=1, batch_size=32), steps=None,
                                          start_delay_secs=1000, throttle_secs=1)
    ...
    if task_type == "export":
        feature_spec = {
            "img_raw": tf.placeholder(dtype=tf.float32, shape=[None, 28, 28], name="img_raw"),
        }
